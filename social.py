...
def tootTheTweet(tweet):
    ...
    tweet_check_file_path = os.getcwd() + helpers._config('toots.cache_path') + tweet['id']
    ...
    if response.status_code == 200:
        ...
        helpers._info('tootTheTweet() => FAIL. Could not post tweet ' + tweet_id + 'to Mastodon.' + str(response.status_code))
        helpers._info('tootTheTweet() => Response: ' + response.text)
